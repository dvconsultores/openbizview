...
<%@ page import="org.eclipse.birt.report.presentation.aggregation.IFragment,
				 ...
				 org.eclipse.birt.report.utility.ParameterAccessor,org.openbizview.util.session,
...
<%-----------------------------------------------------------------------------
	Personalizando mensajes al español para integración Budget View.
    Andrés Dominguez 07/04/2009
-----------------------------------------------------------------------------%>
     <%
     String mensaje = null;
     session a = new session();
     if (fragment.getClientId( )== a.getParam1())
         {
         mensaje = a.getParam();
         }
     else if (fragment.getClientId( )== a.getExpDatos1())
         {
         mensaje = a.getExpDatos();
         }
     else if (fragment.getClientId( )== a.getExpRep1())
         {
         mensaje = a.getExpRep();
         }
     else if (fragment.getClientId( )== a.getImpRep1())
         {
         mensaje = a.getImpRep();
         }
	 %>
				...
				<tr>
					<td>
						<label class="birtviewer_hidden_label" for="<%= fragment.getClientId( ) %>dialogCloseBtn">
							<%= 
								BirtResources.getMessage( "birt.viewer.dialog.close", valorCookie )
							%>
						</label>						
				...
				<div class="dialogBtnBar">
					<div id="<%= fragment.getClientId( ) %>dialogCustomButtonContainer" class="dialogBtnBarButtonContainer">
						<div id="<%= fragment.getClientId( ) %>okButton">
							...
							<input type="button" value="<%= BirtResources.getHtmlMessage( "birt.viewer.dialog.ok", valorCookie ) %>" 
								title="<%= BirtResources.getHtmlMessage( "birt.viewer.dialog.ok", valorCookie ) %>"  
